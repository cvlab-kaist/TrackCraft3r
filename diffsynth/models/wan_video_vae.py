...
class WanVideoVAE(nn.Module):
    ...
    def __init__(self, z_dim=16):
        ...
        self.use_gradient_checkpointing = True  # default: on (backward compat)
        self.z_dim = z_dim
    ...
    def single_encode(self, video, device):
        ...
        if self.training and self.use_gradient_checkpointing:
            x = torch.utils.checkpoint.checkpoint(
                self.model.encode, video, self.scale,
                use_reentrant=False,
            )
        else:
    ...
    def single_decode(self, hidden_state, device):
        ...
        if self.use_gradient_checkpointing and (self.training or hidden_state.requires_grad):
            video = torch.utils.checkpoint.checkpoint(
                self.model.decode, hidden_state, self.scale,
                use_reentrant=False,
            )
        else:

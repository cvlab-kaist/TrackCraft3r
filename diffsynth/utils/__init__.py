    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            ...
            if self.local_model_path is None:
                self.local_model_path = os.environ.get("MODELSCOPE_CACHE", "./checkpoints/wan_models")
            if not skip_download:
                _target = os.path.join(self.local_model_path, self.model_id)
                downloaded_files = glob.glob(self.origin_file_pattern, root_dir=_target)
                if downloaded_files:
                    print(f"  [skip download] {len(downloaded_files)} file(s) found in {_target}")
                else:
                    snapshot_download(
                        self.model_id,
                        local_dir=_target,
                        ...
                        local_files_only=os.environ.get("MODELSCOPE_OFFLINE", "0") == "1"
                    )
